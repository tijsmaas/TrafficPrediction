...
from lib.metrics.metrics_torch import masked_mae_torch
...
class DCRNNSupervisor:
    ...
    def save_model(self, epoch):
        if not os.path.exists('data/metr-la/checkpoints/'):
            os.makedirs('data/metr-la/checkpoints/')

        config = dict(self._kwargs)
        config['model_state_dict'] = self.dcrnn_model.state_dict()
        config['epoch'] = epoch
        torch.save(config, 'data/metr-la/checkpoints/dcrnn_pytorch_%d.tar' % epoch)
        self._logger.info("Saved model at {}".format(epoch))
        return 'data/metr-la/checkpoints/dcrnn_pytorch_%d.tar' % epoch

    def load_model(self):
        self._setup_graph()
        assert os.path.exists('data/metr-la/checkpoints/dcrnn_pytorch_%d.tar' % self._epoch_num), 'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load('data/metr-la/checkpoints/dcrnn_pytorch_%d.tar' % self._epoch_num, map_location='cpu')
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            y_truths = []
            y_preds = []
            for i, (x, y) in tqdm(enumerate(val_iterator)):
                x, y = self._prepare_data(x, y)
                output = self.dcrnn_model(x)
            ...
            # Scale preds so that they can be compared to labels
            y_preds = self.standard_scaler.inverse_transform(y_preds)
            self.print_horizon_results(y_preds.transpose((1,2,0)), y_truths.transpose((1,2,0)))

            return mean_loss, {'prediction': y_preds, 'truth': y_truths}

    def print_horizon_results(self, y_preds, y_truths):
        amae = []
        amape = []
        armse = []
        for i in range(y_preds.shape[2]):
            pred = y_preds[:, :, i]
            real = y_truths[:, :, i]
            mae, rmse, mape = metrics_np.calculate_metrics(pred, real)
            log = 'Evaluate model for horizon {:d}, Test MAE: {:.4f}, Test RMSE: {:.4f}, Test MAPE: {:.4f}'
    ...
    def _train(self, base_lr,
        ...
        print('Train with args:', base_lr,
               steps, patience, epochs, lr_decay_ratio, log_every, save_model,
               test_every_n_epochs, epsilon, kwargs)
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in tqdm(enumerate(train_iterator)):
                optimizer.zero_grad()
    ...
    def _compute_loss(self, y_true, y_predicted):
        ...
        return masked_mae_torch(y_predicted, y_true, null_val=0.0)
